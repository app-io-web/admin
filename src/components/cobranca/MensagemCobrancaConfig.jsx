...
export default function MensagemCobrancaConfig() {
    ...
    async function carregarMensagem() {
      try {
        ...
        if (dados) {
          ...
          setImagemURL(dados?.MAX_FIBRA?.imagem || '');
          setEnviarMidia(dados?.MAX_FIBRA?.enviarMidia || false);
  ...
  const confirmDeleteImagem = async () => {
    ...
    try {
      ...
      // Optionally revert state changes if save fails
  ...
  return (
    <Box maxW="700px" mx="auto" p={4}>
      ...
      {carregando ? (
        ...
        <>
          <Box fontSize="sm" corrobor color="gray.600" mb={2}>
          ...
          {imagemURL && (
            <Box mt={4}>
              ...
              <Box display="flex" alignItems="center" gap={4}>
                <img
                  src={imagemURL || null} // Use null if imagemURL is empty
