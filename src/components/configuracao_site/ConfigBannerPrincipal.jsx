...
export default function ConfigBannerPrincipal() {
  ...
  const fetchBanners = async () => {
    setLoading(true);
    try {
      const res = await fetch('https://api.configsite.nexusnerds.com.br/banners-site');
  ...
  const handleSubmit = async () => {
    try {
      ...
      const url = 'https://api.configsite.nexusnerds.com.br/banners-site';
      ...
      if (res.ok) {
        toast({
          title: editBanner ? 'Banner principal atualizado' : 'Banner principal adicionado',
          status: 'success',
          duration: 3000,
          isClosable: true,
        });
  ...
  const handleDeleteImage = async (bannerId, key) => {
    try {
      const res = await fetch('https://api.configsite.nexusnerds.com.br/banners-site', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ Id: bannerId, [key]: '' }),
      });
      if (res.ok) {
  ...
  return (
    <Box p={4}>
      <HStack justify="space-between" mb={4}>
        <Text fontSize="xl" fontWeight="bold">Configuração de Banner Principal</Text>
                    ...
                    {banner[key] ? (
                      <>
                        ...
                        <Button
                          ...
                          colorScheme="red"
                          mt={2}
                          onClick={() => handleDeleteImage(banner.Id, key)}
                        >
                          Deletar
                        </Button>
                      </>
